/*
* More optimized solution with O(m * n * k) time and O(m * n) space
*/
class Solution {
            ...
            while (isNotEmpty()) {
                ...
                for (d in dir) {
                    ...
                    if (isValid(iNxt, jNxt)) {
                        val nxtObsLeft = if (grid[iNxt][jNxt] == 1) obsLeft - 1 else obsLeft
                        if (nxtObsLeft > visited[iNxt][jNxt]) {
                            if (nxtObsLeft != -1) {
                                visited[iNxt][jNxt] = nxtObsLeft
                                addLast(intArrayOf(iNxt, jNxt, nxtObsLeft, len + 1))
                            }
                        }  
                    }
                }
...
/*
* More intuitive solution with O(m * n * k) time and O(m * n * k) space
*/
class Solution {
    fun shortestPath(grid: Array<IntArray>, k: Int): Int {
        val dir = arrayOf(
